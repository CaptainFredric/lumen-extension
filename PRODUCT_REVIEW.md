...
Review baseline: GitHub commit 824b2a4, version 0.5.0. Subsequent review changes continue after that baseline; it does not identify the current release. Use PRODUCT.md for current product boundaries and document ownership. The local checkout initially contained version 0.2.0 and was 27 commits behind. Changes made against that old checkout are preserved on local main in commit 11f72a0; they were never published and should remain separate from this review.

## Current product

Selected Area review is now implemented. Review-before-save and Private Review
Mode open an extension-owned approval window before capture. It shows geometry
and page checks, not captured pixels. Cancellation, closing, expiry and changed
page geometry save nothing. Settings and saved manual boxes are checked again
after approval. A worker restart invalidates outstanding approval.

Unit tests cover approval ownership, replay, cancellation, expiry and geometry.
The loaded-extension test verifies cancelled and closed windows, rejects a stale
scrolled-page approval, then saves a reviewed rectangle and a lasso under Private
Review Mode. Settings and product-contract tests cover the coordinated vocabulary
change while preserving existing preference keys and optional defaults.

The Store pack now follows one Bug Garden workflow through Result, responsive
views, annotation, Compare and Library. The generator captures both checkout
states through the extension, marks the original in the real editor, and creates
then pauses an actual monitor. Its proof record links capture identifiers and
image hashes. New Page Context records omit guessed CTA and site classification.
The public download is now beta 1 from tested commit a903623, with a retained
checksum and CI record. The site uses one consolidated inspection stylesheet.
Broader module decomposition remains deferred until repeated use identifies a
specific boundary worth extracting.

September consolidation priority: the public Bug Garden now has real responsive
exports, redaction, page context, and a Capture Result screenshot generated by
`npm run proof:garden`. The popup is now a capture launcher; Library owns
monitor administration and Result Details owns retained Page Context. The Store
images now use that same fixture. A new video and final Store submission remain
pending. The roadmap now prioritizes release completion and repeated personal use.
The feature ideas below remain proposals rather than the immediate work queue.

The consolidation preserves capture and content-script mechanics. Launcher,
Library monitor CRUD and alarm updates, capture-specific context, Settings,
responsive exports, selected areas, difficult-site fixtures, lifecycle checks,
and ZIP behavior have automated coverage. The exact package passed all three
native macOS shortcuts after the harness switched from an emulated viewport to
physical screen dimensions. Each path exported a capture and opened Result,
with no persistent site permissions. The toolbar-button click remains a manual check.
Store images have been regenerated. Published video recordings remain older material.
...
The central workflow is capture, review, edit, and save. Completed responsive views now survive a later interruption as a partial library item. Automatic redaction and Private Review Mode start off for fresh installs, with saved preferences preserved.
